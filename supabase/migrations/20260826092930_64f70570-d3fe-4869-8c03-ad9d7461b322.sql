-- Send a single summary email per drop-in purchase with the list of reserved classes.
...
BEGIN
  ...
  LOOP
    ...
    IF FOUND THEN
      v_new_bookings := array_append(v_new_bookings, v_payment.booking_id);
...
-- Cash batch: send one summary email instead of one per booking, and include class details.
