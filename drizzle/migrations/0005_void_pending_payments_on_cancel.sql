...
begin
  ...
  -- A cancelled booking must not keep an open (unpaid) charge around: void the
  -- pending payment rows of this booking and drop the mirrored ledger income
  -- once no sibling booking of the same (cash/bizum) batch is still pending.
  select p.stripe_session_id into v_session
  ...
  if v_session is not null and v_session not like 'cs_%' then
    if not exists (
      select 1 from public.payments p
       where p.stripe_session_id = v_session
         and p.status in ('pending','confirmed')
    ) then
      delete from public.ledger_entries le where le.stripe_session_id = v_session;
    end if;
  end if;
